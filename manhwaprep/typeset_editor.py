...
class TextBoxItem(QGraphicsItem):
    ...
    def _refit(self, top=None, bottom=None, min_h=None):
        ...
        self.font.setPointSizeF(
            max(self.FONT_MIN, min(self.max_size, self.FONT_MAX)))
        cy = self.y() + self.h / 2
        # Measure height with the SAME QTextLayout the fill/outline render through,
        # so the box grows to fit exactly what gets drawn and never clips the last
        # line. (The old fm.boundingRect metric could come out shorter than the
        # rendered text on tall Khmer faces, pushing the bottom lines out of view —
        # and _text_layout already folds in line_spacing.)
        _layout, natural_h = self._text_layout()
    ...
    def _shaped_text_path(self, r: QRectF):
        ...
        y_off = max(0.0, y_off)  # never push text above the box top
        path = QPainterPath()
