...
def test_refit_box_is_tall_enough_to_not_clip_the_last_line():
    """_refit must size the box from the SAME layout the text renders through, so
    an auto-fit box always fits every wrapped line. It used to measure with
    fm.boundingRect, which came out shorter than the rendered text on tall Khmer
    faces — the box was too short and the bottom sentences were chopped."""
    for w, ms in ((300, 30), (180, 34), (150, 44)):
        it = TextBoxItem(1, "នាក់ប្រាក់ ១០០ ថង់ ដ៏ និងគ្រិណាត់ ស្ត្រ ៣០០ ឆ្នាំង",
                         0, 0, w, 100)
        it.max_size = ms
        it._refit()
        # The rendered text height must not exceed the box height _refit chose.
        _layout, total_h = it._text_layout()
        assert total_h <= it.h + 0.5, (
            f"box too short (w={w},{ms}pt): total_h={total_h:.1f} > h={it.h:.1f}")

        # And the actual ink must land inside the box (no top/bottom clip).
        W, H = int(it.w), int(it.h)
        it.fill = QColor(0, 0, 0)
        m, _, _ = _coverage_mask(
            it, lambda p: it._paint_text_body(p, QRectF(0, 0, W, H)))
        ys = [y for y in range(H)
              if any(m.pixelColor(x, y).alpha() > 25 for x in range(0, W, 2))]
        assert ys, "nothing rendered"
        assert min(ys) >= 0 and max(ys) <= H - 1  # ink stays within the box
